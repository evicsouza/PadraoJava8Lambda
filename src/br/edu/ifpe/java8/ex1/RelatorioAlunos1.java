...
public class RelatorioAlunos1 {
	...
	static {
		...
		CONDICAO.put(TipoRelatorio1.APENAS_FINAL, aluno -> aluno.getMedia() < MEDIA_APROVACAO && aluno.getMedia() >= MEDIA_MINIMA_FINAL);
		CONDICAO.put(TipoRelatorio1.APENAS_REPROVADOS_DIRETO, aluno -> aluno.getMedia() < MEDIA_APROVACAO);
	...
	public static void gerar(TipoRelatorio1 opcao) {
		...
		try {
			...
			while (leitor.hasNextLine()) {
				...
				if(CONDICAO.get(opcao).test(aluno)){
